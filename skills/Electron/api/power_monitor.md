---
title: "powerMonitor"
description: "Documentação técnica e referência da API de powerMonitor no Electron."
topics:
  - "Api"
keywords:
  - "powerMonitor"
  ...
  - "critical"
source_scope:
  - "https://www.electronjs.org/pt/docs/latest/api/power-monitor"
---
...
Returns `string` - The system's current idle state. Pode ser `active`, `idle`, `locked` ou `unknown`.
...
Returns `string` - The system's current thermal state. Pode ser `unknown`, `nominal`, `fair`, `serious` ou `critical`.
...
See [`powerMonitor.isOnBatteryPower()`](#powermonitorisonbatterypower).[Editar esta página](https://github.com/electron/electron/edit/main/docs/api/power-monitor.md)

...
keywords:
  ...
  - "BrowserWindow"
source_scope:
  - "https://www.electronjs.org/pt/docs/latest/api/webview-tag"
---
...
Electron's `webview` tag is based on [Chromium's `webview`](https://developer.chrome.com/docs/extensions/reference/webviewTag/), which is undergoing dramatic architectural changes. This impacts the stability of `webviews`, including rendering, navigation, and event routing. We currently recommend to not use the `webview` tag and to consider alternatives, like `iframe`, a [`WebContentsView`](/pt/docs/latest/api/web-contents-view), or an architecture that avoids embedded content altogether.

## Enabling

By default the `webview` tag is disabled in Electron >= 5.  You need to enable the tag by setting the `webviewTag` webPreferences option when constructing your `BrowserWindow`. For more information see the [BrowserWindow constructor docs](/pt/docs/latest/api/browser-window).
...
A `string` which is a comma separated list of strings which specifies the web preferences to be set on the webview. The full list of supported preference strings can be found in [BrowserWindow](/pt/docs/latest/api/browser-window#new-browserwindowoptions).
...
Returns `Promise<void>` - The promise will resolve when the page has finished loading (see [`did-finish-load`](/pt/docs/latest/api/webview-tag#event-did-finish-load)), and rejects if the page fails to load (see [`did-fail-load`](/pt/docs/latest/api/webview-tag#event-did-fail-load)).
...
See [`webContents.adjustSelection`](/pt/docs/latest/api/web-contents#contentsadjustselectionoptions) for examples.
...
- Objeto `options` (opcional)
  ...
  - `findNext` boolean (optional) - Whether to begin a new text finding session with this request. Should be `true` for initial requests, and `false` for follow-up requests. O padrão é `false`.

  - `matchCase` boolean (optional) - Whether search should be case-sensitive, defaults to `false`.

Returns `Integer` - The request id used for the request.

Starts a request to find all matches for the `text` in the web page. The result of the request can be obtained by subscribing to [`found-in-page`](/pt/docs/latest/api/webview-tag#event-found-in-page) event.
...
- Objeto `options` (opcional)
  ...
  - `usePrinterDefaultPageSize` boolean (optional) - Whether to use the system's default page size. Por padrão é `false`. Cannot be combined with `pageSize`. When `deviceName` is provided, uses the default page size of that specific printer. When `deviceName` is not provided, uses the default page size of the system's default printer. If the printer's default page size cannot be retrieved, falls back to A4 (210mm x 297mm).
...
- `options` [PrintToPDFOptions](/pt/docs/latest/api/structures/print-to-pdf-options)
...
`A4`, `A5`, `A6`, `Legal`, `Letter`, `Tabloid`, `Ledger`, or an Object containing `height` and `width` in inches. Defaults to `Letter`.
  ...
  - `margins` [PrintToPDFMargins](/pt/docs/latest/api/structures/print-to-pdf-margins) (optional)
...
- `rect` [Rectangle](/pt/docs/latest/api/structures/rectangle) (optional) - The area of the page to be captured.

Returns `Promise<NativeImage>` - Resolves with a [NativeImage](/pt/docs/latest/api/native-image)
...
Send an asynchronous message to renderer process via `channel`, you can also send arbitrary arguments. The renderer process can handle the message by listening to the `channel` event with the [`ipcRenderer`](/pt/docs/latest/api/ipc-renderer) module.

See [webContents.send](/pt/docs/latest/api/web-contents#contentssendchannel-args) for examples.
...
See [webContents.sendToFrame](/pt/docs/latest/api/web-contents#contentssendtoframeframeid-channel-args) for examples.

### `<webview>.sendInputEvent(event)`

- `event`  [MouseInputEvent](/pt/docs/latest/api/structures/mouse-input-event) | [MouseWheelInputEvent](/pt/docs/latest/api/structures/mouse-wheel-input-event) | [KeyboardInputEvent](/pt/docs/latest/api/structures/keyboard-input-event)

Retorna `Promise<void>`

Sends an input `event` to the page.

See [webContents.sendInputEvent](/pt/docs/latest/api/web-contents#contentssendinputeventinputevent) for detailed description of `event` object.
...
Emitted when there is a new context menu that needs to be handled.[Editar esta página](https://github.com/electron/electron/edit/main/docs/api/webview-tag.md)

...
O Electron possui APIs para a configuração do ícone do aplicativo na barra de tarefas do Windows. This API supports both Windows-only features like [creation of a `JumpList`](#jumplist), [custom thumbnails and toolbars](#thumbnail-toolbars), [icon overlays](#icon-overlays-in-taskbar), and the so-called ["Flash Frame" effect](#flash-frame), and cross-platform features like [recent documents](/pt/docs/latest/tutorial/recent-documents) and [application progress](/pt/docs/latest/tutorial/progress-bar).

## JumpList

Windows allows apps to define a custom context menu that shows up when users right-click the app's icon in the taskbar. Esse menu de contexto é chamado `JumpList`. You specify custom actions in the `Tasks` category of JumpList, as quoted from [MSDN](https://learn.microsoft.com/en-us/windows/win32/shell/taskbar-extensions#tasks):
...
To set user tasks for your application, you can use [app.setUserTasks](/pt/docs/latest/api/app#appsetusertaskstasks-windows) API.

#### Exemplos

##### Set user tasks

Starting with a working application from the [tutorial starter code](/pt/docs/latest/tutorial/tutorial-first-app#final-starter-code), update the `main.js` file with the following lines:
...
To set thumbnail toolbar in your application, you need to use [BrowserWindow.setThumbarButtons](/pt/docs/latest/api/browser-window#winsetthumbarbuttonsbuttons-windows)
...
To set the overlay icon for a window, you need to use the [BrowserWindow.setOverlayIcon](/pt/docs/latest/api/browser-window#winsetoverlayiconoverlay-description-windows) API.
...
To flash the BrowserWindow taskbar button, you need to use the [BrowserWindow.flashFrame](/pt/docs/latest/api/browser-window#winflashframeflag) API.

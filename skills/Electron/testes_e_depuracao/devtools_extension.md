---
title: "Extensão de DevTools"
description: "Electron supports [Chrome DevTools extensions](https://developer.chrome.com/docs/extensions/how-to/devtools/extend-devtools), which can be used to extend the ability of Chrome's de"
topics:
  - "Testes e depuracao"
keywords:
  ...
  - "ready"
source_scope:
  - "https://www.electronjs.org/pt/docs/latest/tutorial/devtools-extension"
---
...
If you don't want to use the tooling approach, you can also do all of the necessary operations by hand. To load an extension in Electron, you need to download it via Chrome, locate its filesystem path, and then load it into your [Session](/pt/docs/latest/api/session) by calling the [`ses.loadExtension`](/pt/docs/latest/api/extensions-api#extensionsloadextensionpath-options) API.
...
Pass the location of the extension to the [`ses.loadExtension`](/pt/docs/latest/api/extensions-api#extensionsloadextensionpath-options) API. For React Developer Tools `v4.9.0`, it looks something like:
...
- `loadExtension` returns a Promise with an [Extension object](/pt/docs/latest/api/structures/extension), which contains metadata about the extension that was loaded. This promise needs to resolve (e.g. with an `await` expression) before loading a page. Otherwise, the extension won't be guaranteed to load.
...
You can pass the extension's ID to the [`ses.removeExtension`](/pt/docs/latest/api/extensions-api#extensionsremoveextensionextensionid) API to remove it from your Session. Loaded extensions are not persisted between app launches.

## DevTools extension support

Electron only supports [a limited set of `chrome.*` APIs](/pt/docs/latest/api/extensions), so extensions using unsupported `chrome.*` APIs under the hood may not work.
...
If the extension works on Chrome but not on Electron, file a bug in Electron's [issue tracker](https://github.com/electron/electron/issues) and describe which part of the extension is not working as expected.[Editar esta página](https://github.com/electron/electron/edit/main/docs/tutorial/devtools-extension.md)

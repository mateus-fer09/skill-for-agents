...
description: "Inter-process communication (IPC) is a key part of building feature-rich desktop applications in Electron. Because the main and renderer processes have different responsibilities i"
topics:
...
In Electron, processes communicate by passing messages through developer-defined "channels" with the [`ipcMain`](/pt/docs/latest/api/ipc-main) and [`ipcRenderer`](/pt/docs/latest/api/ipc-renderer) modules. These channels are **arbitrary** (you can name them anything you want) and **bidirectional** (you can use the same channel name for both modules).
...
Before proceeding to implementation details, you should be familiar with the idea of using a [preload script](/pt/docs/latest/tutorial/process-model#preload-scripts) to import Node.js and Electron modules in a context-isolated renderer process.

- For a full overview of Electron's process model, you can read the [process model docs](/pt/docs/latest/tutorial/process-model).

- For a primer into exposing APIs from your preload script using the `contextBridge` module, check out the [context isolation tutorial](/pt/docs/latest/tutorial/context-isolation).

## Pattern 1: Renderer to main (one-way)

To fire a one-way IPC message from a renderer process to the main process, you can use the [`ipcRenderer.send`](/pt/docs/latest/api/ipc-renderer) API to send a message that is then received by the [`ipcMain.on`](/pt/docs/latest/api/ipc-main) API.
...
[docs/fiddles/ipc/pattern-1 (43.4.0)](https://github.com/electron/electron/tree/v43.4.0/docs/fiddles/ipc/pattern-1)[Open in Fiddle](https://fiddle.electronjs.org/launch?target=electron/v43.4.0/docs/fiddles/ipc/pattern-1)
...
The above `handleSetTitle` callback has two parameters: an [IpcMainEvent](/pt/docs/latest/api/structures/ipc-main-event) structure and a `title` string. Whenever a message comes through the `set-title` channel, this function will find the BrowserWindow instance attached to the message sender and use the `win.setTitle` API on it.
...
We don't directly expose the whole `ipcRenderer.send` API for [security reasons](/pt/docs/latest/tutorial/context-isolation#security-considerations). Make sure to limit the renderer's access to Electron APIs as much as possible.
...
A common application for two-way IPC is calling a main process module from your renderer process code and waiting for a result. This can be done by using [`ipcRenderer.invoke`](/pt/docs/latest/api/ipc-renderer#ipcrendererinvokechannel-args) paired with [`ipcMain.handle`](/pt/docs/latest/api/ipc-main#ipcmainhandlechannel-listener).
...
For this demo, you'll need to add code to your main process, your renderer process, and a preload script. The full code is below, but we'll be explaining each file individually in the following sections.
[docs/fiddles/ipc/pattern-2 (43.4.0)](https://github.com/electron/electron/tree/v43.4.0/docs/fiddles/ipc/pattern-2)[Open in Fiddle](https://fiddle.electronjs.org/launch?target=electron/v43.4.0/docs/fiddles/ipc/pattern-2)
...
We don't directly expose the whole `ipcRenderer.invoke` API for [security reasons](/pt/docs/latest/tutorial/context-isolation#security-considerations). Make sure to limit the renderer's access to Electron APIs as much as possible.
...
When sending a message from the main process to a renderer process, you need to specify which renderer is receiving the message. Messages need to be sent to a renderer process via its [`WebContents`](/pt/docs/latest/api/web-contents) instance. This WebContents instance contains a [`send`](/pt/docs/latest/api/web-contents#contentssendchannel-args) method that can be used in the same way as `ipcRenderer.send`.

To demonstrate this pattern, we'll be building a number counter controlled by the native operating system menu.

For this demo, you'll need to add code to your main process, your renderer process, and a preload script. The full code is below, but we'll be explaining each file individually in the following sections.
[docs/fiddles/ipc/pattern-3 (43.4.0)](https://github.com/electron/electron/tree/v43.4.0/docs/fiddles/ipc/pattern-3)[Open in Fiddle](https://fiddle.electronjs.org/launch?target=electron/v43.4.0/docs/fiddles/ipc/pattern-3)
...
We don't directly expose the whole `ipcRenderer.on` API for [security reasons](/pt/docs/latest/tutorial/context-isolation#security-considerations). Make sure to limit the renderer's access to Electron APIs as much as possible. Also don't just pass the callback to `ipcRenderer.on` as this will leak `ipcRenderer` via `event.sender`. Use a custom handler that invokes the `callback` only with the desired arguments.
...
In the case of this minimal example, you can call `ipcRenderer.on` directly in the preload script rather than exposing it over the context bridge.preload.js (Preload Script)
...
- Pass a [MessagePort](/pt/docs/latest/tutorial/message-ports) from the main process to both renderers. This will allow direct communication between renderers after the initial setup.
...
In particular, DOM objects (e.g. `Element`, `Location` and `DOMMatrix`), Node.js objects backed by C++ classes (e.g. `process.env`, some members of `Stream`), and Electron objects backed by C++ classes (e.g. `WebContents`, `BrowserWindow` and `WebFrame`) are not serializable with Structured Clone.[Editar esta página](https://github.com/electron/electron/edit/main/docs/tutorial/ipc.md)[AnteriorContext Isolation](/pt/docs/latest/tutorial/context-isolation)[AvançarProcess Sandboxing](/pt/docs/latest/tutorial/sandbox)
...
- [Pattern 1: Renderer to main (one-way)](#pattern-1-renderer-to-main-one-way)
  ...
  - [1. Listen for events with `ipcMain.on`](#1-listen-for-events-with-ipcmainon)
  - [2. Expose `ipcRenderer.send` via preload](#2-expose-ipcrenderersend-via-preload)
  - [3. Build the renderer process UI](#3-build-the-renderer-process-ui)

- [Pattern 2: Renderer to main (two-way)](#pattern-2-renderer-to-main-two-way)

  - [1. Listen for events with `ipcMain.handle`](#1-listen-for-events-with-ipcmainhandle)
  - [2. Expose `ipcRenderer.invoke` via preload](#2-expose-ipcrendererinvoke-via-preload)
  - [3. Build the renderer process UI](#3-build-the-renderer-process-ui-1)
...
- [Pattern 3: Main to renderer](#pattern-3-main-to-renderer)
  ...
  - [1. Send messages with the `webContents` module](#1-send-messages-with-the-webcontents-module)
  - [2. Expose `ipcRenderer.on` via preload](#2-expose-ipcrendereron-via-preload)
  - [3. Build the renderer process UI](#3-build-the-renderer-process-ui-2)

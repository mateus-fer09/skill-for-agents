...
description: "In addition to the default block types that BlockNote offers, you can also make your own custom blocks using React components. Take a look at the demo below, in which we add a cust"
topics:
...
### [Block Config (`CustomBlockConfig`)](#block-config-customblockconfig)
...
*In the alert demo, we want the user to be able to type rich text in our
alert, so we set `content` to `"inline"`.*
...
*In the alert demo, we add a `type` prop for the type of alert that we want
...
Properties](/docs/features/blocks#default-block-properties).*

### [Block Implementation (`ReactCustomBlockImplementation`)](#block-implementation-reactcustomblockimplementation)
...
### [Block Config Options](#block-config-options)
...
## [Adding Custom Blocks to the Editor](#adding-custom-blocks-to-the-editor)
...
Learn how to create custom schemas for your BlockNote editor](/docs/features/custom-schemas)[
